using Fury.Characters.Bodies;
using Fury.Characters.Motors;
...
namespace Fury.Characters.Weapons {
    ...
    public class WeaponHandler : NetworkBehaviour {
        ...
        private void Weapon_OnThrownInstantiateRequired(object sender, ThrownInstantiateEventArgs e) {
            ...
            throwable.Initialize(e.PreciseTick, e.Direction * e.Force, e.ThrownBy);
        }
        ...
        private void Fire(PreciseTick pt, Vector3 position, Vector3 forward) {
            ...
            Weapon.Fire(pt, position, forward, ReturnNetworkRoles(), _userData);
            ...
            if(Weapon.WeaponType == WeaponTypes.Raycast) {
                ...
                if(Physics.Raycast(ray, out hit, float.PositiveInfinity, (GlobalManager.LayerManager.DefaultLayer | GlobalManager.LayerManager.HitboxLayer))) {
                    ...
                    if(base.IsServer) {
                        ...
                        UserData hitBoxTeam = hit.collider.GetComponentInParent<UserData>();
                        if(hitBoxTeam)
                            Debug.Log($"Player {_userData.userName} of team {_userData.team} hitted player of Team {hitBoxTeam.team}");
                        if(hitbox != null && _userData.team != hitBoxTeam.team) {
                            hitbox.Hit(Weapon.Damage);
                            GetComponent<UserData>().UpdateScore(Weapon.Damage);
                        }
                    }
                }
            }
                ...
                Collider[] hits = Physics.OverlapSphere(position + (forward * Weapon.ReturnMeleeDistance()), Weapon.ReturnMeleeRadius(), (GlobalManager.LayerManager.DefaultLayer | GlobalManager.LayerManager.HitboxLayer));
                {
                    for(int i = 0; i < hits.Length; i++) {
                        ...
                        if(base.IsServer) {
                            ...
                            UserData hitBoxUserData = hits[i].GetComponent<UserData>();
                            if(hitbox != null) {
                                ...
                                if(angle >= 0f && angle <= 45f)
                                    modifiedDamage *= 2;
                                if(hitBoxUserData != null) {
                                    Debug.Log($"Player {_userData.userName} of team {_userData.team} Melee Attacked player of Team {hitBoxUserData.team}");
                                    if(_userData.team != hitBoxUserData.team) {
                                        hitbox.Hit(modifiedDamage);
                                        GetComponent<UserData>().UpdateScore(modifiedDamage);
                                    }
                                }
                            }
                        }
